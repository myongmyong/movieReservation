...
<div id="wrapper">
	...
	<div class="container">
	    <div id="main">
            ...
            <div id="bodyDiv">
				<h2 style="padding:2em;">[예매확인] 비회원 로그인</h2>
				<form action="<%=contextPath %>/login/guestLogin.do" method="post" name="guestLoginForm" id="guestLoginForm">
					<table>
						<tr>
						...
						<tr>
							...
							<td>
								<select name="cellphoneInit" id="cellphoneInit">
...
function noULoginClick() {
	var noUNm = $('#noUNm').val();
	var noUBirth = $('#noUBirth').val();
	var noUMobileCarrier = $('#noUMobileCarrier').val();
	var noUHp = $('#cellphoneInit').val()+$('#cellphoneMiddle').val()+$('#cellphoneEnd').val();
	var noUPw = $('#noUPassword').val();	

	$.ajax({
		url: '<%=contextPath%>/login/guestChkLogin.do',
		type : 'post',
		datatype : 'json',
		data : {
				"noUNm" : noUNm
				,"noUBirth" : noUBirth
				,"noUMobileCarrier" : noUMobileCarrier
				,"noUHp" : noUHp
				,"noUPw" : noUPw
		},
		success : function(data) {
				if (data.noUId != null) {
					...
					alert('입력하신 정보에 해당하는 예매정보가 없습니다.');
	...
	} else if( $('#cellphoneMiddle').val().length < 3 || $('#cellphoneEnd').val().length < 4) {
		alert('휴대폰 번호를 정확히 입력해 주세요.');
		window.location.reload(true);
	} else if ($('#noUPassword').val().length < 4) {
		alert('비밀번호를 확인하세요.(숫자4자리)');
		$('#noUPassword').val('');
		$('#noUPasswordCon').val('');
		$('#noUPassword').focus();
	}
